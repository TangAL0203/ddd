#ifndef _UNDISTORT_CUDA_HPP
#define _UNDISTORT_CUDA_HPP
...
class UndistortImagesCuda
{
private:
    // Camera parameters
    // cv::Mat cameraMatrix = (Mat_<float>(3, 3) << 931.9622, 0., 935.8878, 0., 938.0016, 516.46242, 0, 0, 1);
    cv::Mat cameraMatrix = (Mat_<float>(3, 3) << 931.9622 * 0.5, 0., 935.8878 * 0.5, 0., 938.0016 * 0.5, 516.46242 * 0.5, 0, 0, 1);
    cv::Mat distCoeffs = (Mat_<float>(4, 1) << -0.01875662, 0.01232133, -0.02096538, 0.0072541);
    const int imgWidth = 1920 / 2;
    const int imgHeight = 1080 / 2;
